...
test.describe("Page Navigation", () => {
  ...
  test("should switch between pages when navigating to different URLs", async ({
    ...
    await expect(page).toHaveURL(new RegExp(`/db/${dbId}/pages/.*`));
    ...
    await page2Link.click();
    await expect(page).toHaveURL(new RegExp(`/db/test_db_id/pages/.*`));
    await expect(page.getByRole("heading", { name: page2Title })).toBeVisible();
  });
  ...
  test.afterEach(async ({ page, dbId }) => {
    ...
    await expect(
      page.getByRole("heading", { name: page1Title }),
    ).not.toBeVisible();
    ...
    await page.getByRole("menuitem", { name: "Delete page" }).click();
    await expect(
      page.getByRole("heading", { name: page2Title }),
    ).not.toBeVisible();
  });
});
