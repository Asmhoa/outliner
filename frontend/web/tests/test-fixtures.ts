...
const test = base.extend<{
  ...
  dbId: async ({ page }, use) => {
    ...
    const response = await page.request.get('http://127.0.0.1:8000/databases');
    const databases = await response.json();
    
    // Find the database with name 'playwright_test_db'
    const testDb = databases.find((db: any) => db.name === 'playwright_test_db');
    
    if (!testDb) {
      throw new Error("Test database 'playwright_test_db' not found. Please run test setup first.");
    }
