...
def test_update_block_parent_success(override_get_db):
    ...
    response = client.put("/blocks/parent", json={"block_id": block_id, "new_page_id": new_page_id})
...
def test_delete_workspace_not_found(override_get_db):
    ...
    response = client.delete("/workspaces/999999")  # Use a large number unlikely to exist
...
def test_add_block_with_parent(override_get_db):
    ...
    response = client.put("/blocks/parent", json={
        "block_id": block_id,
    ...
    assert updated_block_data[2] is None  # Should no longer be directly associated with page
